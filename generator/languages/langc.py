...
def define_shared_serializers(service, outfile):
    ...
    outfile.write("    uint32_t length = string != NULL ? (uint32_t)strlen(string) : 0;\n")
    outfile.write("    *((uint32_t*)&buffer->data[buffer->index]) = length;\n")
    outfile.write("    if (length == 0) {\n")
    outfile.write("        buffer->data[buffer->index + sizeof(uint32_t)] = 0;\n")
    outfile.write("        buffer->index += sizeof(uint32_t) + 1;\n")
    outfile.write("        return;\n")
    outfile.write("    }\n")
    outfile.write("    memcpy(&buffer->data[buffer->index + sizeof(uint32_t)], string, length);\n")
    outfile.write("    buffer->data[buffer->index + sizeof(uint32_t) + length] = 0;\n")
    outfile.write("    buffer->index += (sizeof(uint32_t) + length + 1);\n")
    outfile.write("}\n\n")
    outfile.write(
        "static inline void deserialize_string_copy(gracht_buffer_t* buffer, char* out, uint32_t maxLength) { \n")
    outfile.write("    uint32_t length = *((uint32_t*)&buffer->data[buffer->index]);\n")
    outfile.write("    uint32_t clampedLength = GRMIN(length, maxLength - 1);\n")
    outfile.write("    if (length > 0) {\n")
    outfile.write("        memcpy(out, &buffer->data[buffer->index + sizeof(uint32_t)], clampedLength);\n")
    outfile.write("    }\n")
    ...
    outfile.write("    uint32_t length = *((uint32_t*)&buffer->data[buffer->index]);\n")
    outfile.write("    char*    string = NULL;\n")
    outfile.write("    if (length > 0) {\n")
    outfile.write("        string = &buffer->data[buffer->index + sizeof(uint32_t)];\n")
    outfile.write("    }\n")
    outfile.write("    buffer->index += sizeof(uint32_t) + length + 1;\n")
